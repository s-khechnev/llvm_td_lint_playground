...
module Collect_out_info = struct
  ...
  let rec helper src_id =
    let f src_args dst_id =
      let rec get_arg = function
        ...
        | E_aux (E_app (Id_aux (Id id, _), args), _) -> (
            helper id;
            match args with [ x ] -> get_arg x | _ -> "noname_!")
        | _ ->
            (* Format.printf "%a\n"
      ...
      | Some (dst_args, _) ->
          let args =
            List.map2 (fun dst src -> (dst, get_arg src)) dst_args src_args
          in
  ...
  let result : (string, string list) Hashtbl.t = Hashtbl.create 1000

  let saturate_graph () =
    Hashtbl.add result "rX_bits" [ "i" ];
    Hashtbl.add result "wX_bits" [ "i" ];
    Hashtbl.add result "rF_bits" [ "i" ];
    Hashtbl.add result "wF_bits" [ "i" ];
    Hashtbl.add result "write_vreg" [ "vrid" ];
    Hashtbl.add result "read_vreg" [ "vrid" ];
    Hashtbl.add result "write_vmask" [ "vrid" ];
    Hashtbl.add result "read_vmask" [ "vrid" ];
    Hashtbl.add result "read_vmask_carry" [ "vrid" ];
    Hashtbl.add result "read_vreg_seg" [ "vrid" ];
    Hashtbl.add result "write_single_element" [ "vrid" ];
    Hashtbl.add result "read_single_element" [ "vrid" ];
    ...
    let rec dfs v =
      if G.mem_vertex g v then
        G.iter_succ_e
          ...
          g v
    in
    Seq.iter (fun k -> dfs k) (Hashtbl.to_seq_keys result);
...
let dump_execute () =
  let open Myast in
  let process_execute name args body =
    (* Collect_out_info.analyze_execute name body *)
    (* print_endline name; *)
       ...
       let in_args =
         match
           List.find_all
             ...
             args
         with
         | xs when not (String.equal "VMVRTYPE" name) -> ref xs
         | _ -> ref []
       in
       let out_args = ref [] in
       let aliases = Hashtbl.create 20 in
       Collect_out_info.analyze_execute name exp;
       ([], []) *)
  in
              ...
              | P_tuple args ->
                  List.map
                    (function
                      ...
                      | P_aux (P_typ (_, P_aux (P_id (Id_aux (Id id, _)), _)), _)
                        ->
                          id
                      | P_aux (P_lit (L_aux (L_hex hex, _)), _) -> "0x" ^ hex
                      | P_aux (P_wild, _) -> "_"
                      | _ ->
                          (* Format.printf "%a"
                               (Myast.pp_pat_aux Myast.pp_tannot)
                               paux;
                             Format.printf "Arg's extraction fail\n"; *)
                          "FIX ME")
                    args
              | P_lit (L_aux (L_unit, _)) -> [ "unit" ]
              | P_id (Id_aux (Id id, _)) -> [ id ]
              | P_wild -> [ "_" ]
              | P_typ (Typ_aux _, P_aux (P_id (Id_aux (Id id, _)), _)) -> [ id ]
              | P_typ
                  ( Typ_aux _,
                    P_aux (P_typ (_, P_aux (P_id (Id_aux (Id id, _)), _)), _) )
                ->
                  [ id ]
              | P_typ (Typ_aux _, P_aux (P_lit (L_aux (L_unit, _)), _)) ->
                  [ "()" ]
              | P_lit (L_aux (L_hex n, _)) -> [ "0x" ^ n ]
              | P_var (P_aux (P_id (Id_aux (Id _, _)), _), _) -> [ "_" ]
              | _ ->
                  Format.printf "%a" (Myast.pp_pat_aux Myast.pp_tannot) paux;
                  failwithf "Arg's extraction fail\n"
            in
  ...
  let () =
    Queue.iter
      (fun (id, args, body) -> process_execute id args body)
      execute_ids
  in
